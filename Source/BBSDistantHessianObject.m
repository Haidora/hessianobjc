...
+ (BBSDistantHessianObject *)proxyWithProtocol:(Protocol *)aProtocol
                             ...
                             remoteClassPrefix:(NSString *)remoteClassPrefix
{
    BBSDistantHessianObject *heesianObject =
        [[BBSDistantHessianObject alloc] initWithProtocol:aProtocol
...
{
#ifdef DEBUG
    NSLog(@"Forward method %@ for proxy %@", NSStringFromSelector([invocation selector]),
...
#ifdef DEBUG
        NSLog(@"\nparam%i(%@):\n%@\n", (i - 2), NSStringFromClass([objectValue class]),
              objectValue);
#endif
    }
#ifdef DEBUG
...
#endif
    __autoreleasing id result = [proxy callSynchronous:methodName
                                        withParameters:arguments
                                       timeoutInterval:_timeoutInterval];
    [self setReturnValue:result invocation:invocation];
}

- (id)getObjectValueAtIndex:(int *)pIndex
